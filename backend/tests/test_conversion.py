...
def test_reverting_a_conversion_whose_lots_are_already_sold_is_an_error():
    ...
    Отмена стоит здесь на дате более поздней, чем продажа, — форма, которую сама
    служба решений не порождает: её зеркальные записи несут дату отменяемого
    решения и потому ложатся раньше любой позднейшей операции. Через службу этот
    случай ловится проверкой до записи (см. _operations_after), а движковая
    проверка остаётся страховкой для записей, порождённых не службой.
    """
