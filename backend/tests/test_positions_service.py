...
def test_full_redemption_read_from_db_closes_the_position(session):
    """Через настоящую БД, а не только на движке: движок сравнивает тип
    операции через `is`, и проверять это обязан тот тип, что прочитан из базы, а
    не собранный в памяти.

    Тест написан по живому случаю: пока op_type хранился строкой (String(24)),
    из базы он приходил как `str`, `is OperationType.REDEMPTION` молча давало
    ложь — погашение не закрывало позицию, и погашенные облигации оставались в
    портфеле, — а юнит-тест на объекте из памяти при этом проходил. Колонка
    стала нативным enum (миграция 0015), но проверка нужна прежней: она сторожит
    не тип колонки, а то, что чтение из базы возвращает именно OperationType."""
    account = setup_account(session)
    instrument = add_instrument(session)
