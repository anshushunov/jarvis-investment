...
def test_check_prints_reconciliation_of_parts(session, account):
    ...
    # Деньги — слагаемое тождества, а не молчаливая поправка: без него сумма
    # разрезов расходилась с прибылью портфеля на переоценку остатка.
    assert "Деньги и металлы" in text
    # «Прочее» сверяется с журналом прямым запросом, а не печатается дважды из
    # одного кода: признак готовности требует именно сверки.
    assert "Сверка «Прочего» прямым запросом" in text
