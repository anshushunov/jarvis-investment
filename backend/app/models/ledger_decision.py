...
# Ключ, под которым запись отмены называет отменяемое решение. Движок по нему
# раскручивает след того решения — те самые партии, с их датами открытия и
# себестоимостью, — вместо того чтобы подбирать партии встречной операцией по
# FIFO. Подбор по FIFO возвращал верные количества с чужими датами и чужой
# ценой: после отмены конвертации трёхлетняя льгота оказывалась приписана не
# той бумаге, а отмена поправки съедала старую партию брокера.
DECISION_REVERTS_PAYLOAD_KEY = "reverts_decision_id"
