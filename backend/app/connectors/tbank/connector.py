    ...
    def fetch_positions(self, account_external_id: str) -> list[BrokerPosition]:
        ...
        for item in raw_positions:
            ...
            if instrument is None or not instrument.isin:
                ...
                #
                # Исключение — деньги и драгоценные металлы: GetPortfolio
                # отдаёт их псевдо-инструментами (RUB000UTSTOM, USD000UTSTOM,
                # GLDRUB_TOM и подобными, см. докстринг fetch_cash) с
                # настоящим FIGI, но без ISIN — у них его нет и не может быть
                # по природе, это не потеря справочника. instrumentType у
                # таких строк — "currency" (тем же способом их отличает
                # KIND_BY_INSTRUMENT_TYPE выше). Не молчать здесь значило бы
                # получать это предупреждение на каждую денежную позицию
                # каждого счёта каждые пятнадцать минут — сотни в сутки,
                # ровно в том логе, где нужно искать настоящую потерянную
                # бумагу.
