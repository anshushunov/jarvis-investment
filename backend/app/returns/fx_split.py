...
class Split:
    """Нереализованная прибыль открытой позиции и её разложение.

    `total` — бумажная прибыль открытых партий за всё время владения, а НЕ
    прибыль за период: реализованный результат частичных продаж в неё не входит
    (он уезжает в 4c к налогам). У бумаги, которую продавали по частям, это
    разные числа — замер 14.08.2026 у Озона: 70 568 ₽ прибыли периода против
    53 555 ₽ нереализованной.
...
def split_position(lots: list[OpenLot], price: LatestPrice | None, price_currency: str,
    ...
    """Разложение НЕРЕАЛИЗОВАННОЙ прибыли позиции по её открытым партиям.

    Считается бумажная прибыль того, что лежит в портфеле сейчас: `Q·(P₁·R₁ −
    P₀·R₀)` по каждой оставшейся партии. Закрытые партии функция не видит по
    устройству, поэтому с прибылью за период (`InstrumentRow.profit`) величина
    совпадает только у бумаги, которую ни разу не продавали по частям. Сверять
    части нужно с ней самой (`Split.total`), а не с прибылью периода — иначе
    проверка тревожит там, где всё верно (дизайн, разделы 4.4 и 7).
    ...
    for lot in lots:
        ...
        # Количество — остаток партии: проданное отсюда уже вычтено, и потому
        # обе части — про бумажную прибыль открытого, а не про итог владения.
