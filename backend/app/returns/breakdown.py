...
def instrument_and_class_rows(session: Session, book: RateBook, period: Period,
    ...
    return sorted(rows, key=_row_order), sorted(by_class, key=lambda row: row.asset_class)


def _row_order(row: InstrumentRow) -> tuple:
    """Порядок строк разреза по бумагам — от самой значимой для владельца.

    Порядок задаётся здесь, а не на экране: у API один ответ и несколько
    потребителей (экран «Аналитика», прогон `check.py`), и разный порядок у
    каждого означал бы, что сверять их построчно нельзя. `select(Instrument)`
    без `ORDER BY` даёт порядок произвольный и способный меняться от запроса к
    запросу — 253 строки живого портфеля в такой таблице читать невозможно, а
    признак готовности «два прогона дают одинаковые числа» на порядок строк не
    распространяется вовсе.

    Значимость по убыванию, четырьмя ключами:

    1. **Открытые раньше закрытых.** У открытой позиции решение ещё впереди, у
       закрытой всё решено — это разные разделы одной таблицы, и мешать их
       значило бы прятать сегодняшний портфель среди истории (193 закрытые
       бумаги против 59 открытых на замере 14.08.2026).
    2. **Стоимость по убыванию.** Первый вопрос владельца к открытой позиции —
       сколько в ней денег: строка на 2 млн ₽ важнее строки на 2 000 ₽ при
       любой их доходности.
    3. **Модуль прибыли по убыванию.** У закрытых позиций стоимость нулевая у
       всех, и упорядочивает их только результат. Модуль, а не сама прибыль:
       убыток в 200 тыс. ₽ требует внимания ровно так же, как прибыль в
       200 тыс. ₽, и прятать его в хвосте таблицы нельзя.
    4. **Имя, затем идентификатор.** Хвостовой разрыв полного равенства:
       без него две одинаковые по цифрам строки могли бы меняться местами
       между запросами, и «одинаковые числа» снова зависели бы от удачи.

    Неизвестные стоимость и прибыль (нет цены, курса или истории — `None`)
    считаются здесь нулём НАМЕРЕННО: это про место в таблице, а не про
    величину. Такая бумага уезжает в конец своей половины, где её и объясняет
    названная причина; подставить ей ноль в саму строку было бы враньём, а
    порядок сортировки ничего не утверждает о деньгах.
    """
    return (row.closed, -(row.value or Decimal("0")),
            -abs(row.profit or Decimal("0")), row.name, row.instrument_id)
