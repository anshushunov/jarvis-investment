...
# Базовая валюта портфеля: в ней считаются совокупный капитал и все разбивки.
# Позиции и остатки в USD, HKD, CNY и металлах переводятся в неё по курсу на
# дату оценки (app/marketdata/fx.py); то, чему курса не нашлось, в итог не
# входит вовсе и называется отдельно — молча складывать валюты значило бы
# завышать капитал. Константа лежит здесь, а не в аналитике, потому что от неё
# зависят обе стороны: и подсчёт итогов, и решение, какие котировки вообще
# годятся для оценки (app/marketdata/service.py).
BASE_CURRENCY = "RUB"
