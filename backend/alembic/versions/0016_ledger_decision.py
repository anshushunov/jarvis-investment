...
def downgrade() -> None:
    ...
    if orphans:
        raise RuntimeError(
            ...
            "останутся без объяснения. Отмена решений тут не поможет: она "
            "порождает не удаление, а новые записи с тем же source='manual', и "
            "счётчик только вырастет. Выход один — сохранить объяснения вне "
            "базы (выгрузить ledger_decision в файл) и снять ограничение "
