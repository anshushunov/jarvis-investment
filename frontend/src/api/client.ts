...
export interface InstrumentReturn {
  // Ключ строки на экране: тикеры у 252 бумаг живого портфеля не уникальны —
  // редомициляция даёт пару с одним тикером и одним именем эмитента.
  ...
  closed: boolean;
  // Нереализованная прибыль открытых партий и её валютная часть. Отдельно от
  // profit: это разные числа (дизайн, раздел 4.4).
  unrealized: string | null;
  fx_part: string | null;
  reason: string | null;
  // Ценовая часть (`price_part`) в ответе есть, но здесь не объявлена
  // намеренно: экран показывает нереализованную прибыль и её валютную часть, а
  // ценовая — это их разность, и рисовать её третьей колонкой значит занимать
  // место числом, которое уже на экране. Тип объявляет только то, что экран
  // читает; на бэкенде поле живое — по нему прогон сверяет точность разложения
  // (app/returns/check.py, признак готовности, пункт 2).
}
